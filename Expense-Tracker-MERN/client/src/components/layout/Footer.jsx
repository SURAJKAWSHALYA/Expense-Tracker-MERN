...
export default function Footer() {
  return (
    <footer className="mx-auto w-full max-w-7xl px-6 pb-12 pt-4 md:px-8">
      <div className="rounded-[32px] border border-slate-200/80 dark:border-white/10 bg-white dark:bg-slate-900/40 p-10 text-slate-700 dark:text-slate-300 shadow-[0_24px_60px_rgba(15,23,42,0.08)] dark:shadow-2xl backdrop-blur-md sm:p-12 transition-colors duration-300">
        {/* Top CTA strip */}
        <div className="flex flex-col gap-6 border-b border-slate-200 dark:border-white/10 pb-10 md:flex-row md:items-center md:justify-between transition-colors">
          <div>
        ...
        <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm leading-6 text-slate-600 dark:text-slate-400 transition-colors">
            ...
            <div className="flex gap-3">
              {[
                ...
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 dark:border-white/20 bg-slate-50 dark:bg-white/5 text-slate-500 dark:text-slate-400 shadow-sm transition hover:border-indigo-600 hover:text-indigo-600 dark:hover:border-yellow-400 dark:hover:text-yellow-400"
                >
                  <Icon size={14} />
                </a>
              ))}
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              ...
              <ul className="mt-4 space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-slate-600 dark:text-slate-400 transition hover:text-indigo-600 dark:hover:text-yellow-400">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        {/* Bottom bar */}
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-slate-200 dark:border-white/10 pt-8 text-xs text-slate-600 sm:flex-row transition-colors">

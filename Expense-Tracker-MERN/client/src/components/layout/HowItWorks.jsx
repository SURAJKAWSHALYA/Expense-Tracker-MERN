    ...
    >
      <div className="rounded-[32px] border border-white/10 bg-slate-900/40 p-10 shadow-2xl backdrop-blur-md sm:p-14">
        ...
        <motion.div variants={itemVariants} className="mb-14 text-center">
          <p className="text-sm uppercase tracking-[0.26em] text-yellow-400">Simple process</p>
        ...
        <div className="relative grid gap-8 lg:grid-cols-4">
          ...
          <div className="absolute top-8 left-0 right-0 hidden h-px bg-gradient-to-r from-transparent via-white/20 to-transparent lg:block" style={{ top: '2.75rem' }} />
          ...
          {steps.map((step) => {
            ...
            return (
              <motion.div key={step.step} variants={itemVariants} className="relative flex flex-col items-center gap-5 text-center">
                {/* Step circle */}
                <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-fuchsia-500 to-cyan-500 text-white shadow-lg z-10">
                  <Icon size={22} />
                  <span className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-slate-950 border border-white/20 text-[10px] font-bold text-yellow-400 shadow-sm">
                    {step.step.replace('0', '')}
                  </span>
                </div>
